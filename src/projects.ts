...
export function normalizeProject(input: ProjectInput): ProjectSnapshot {
  if (!input || typeof input !== "object") throw new TypeError("project is required");
...
export class ProjectRegistry {
  ...
  create(input: ProjectInput): ProjectSnapshot {
    if (this.#projects.size >= MAX_PROJECTS) throw new RangeError(`registry capacity ${MAX_PROJECTS} reached`);
  ...
  transition(projectId: string, actorId: string, status: ProjectStatus): ProjectSnapshot {
    ...
    if (current.status === "completed" && status !== "completed") throw new Error("completed project is terminal");
