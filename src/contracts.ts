...
function normalize(input: ContractInput): ContractSnapshot {
  ...
  if (title.length < 1 || title.length > 160) throw new Error("invalid contract title");
  if (!Array.isArray(input.partyIds) || input.partyIds.length < 2 || input.partyIds.length > MAX_PARTIES) {
    throw new Error(`partyIds must contain 2-${MAX_PARTIES} entries`);
  }
  const partyIds = input.partyIds.map((value) => safeId("party id", value));
  if (new Set(partyIds).size !== partyIds.length) throw new Error("duplicate party id");
...
function clone(record: ContractSnapshot): ContractSnapshot {
  return Object.freeze({ ...record, partyIds: Object.freeze([...record.partyIds]) });
}
...
export class ContractRegistry {
  ...
  create(input: ContractInput): ContractSnapshot {
    if (this.#contracts.size >= MAX_CONTRACTS) throw new Error("contract capacity reached");
    const record = normalize(input);
    if (this.#contracts.has(record.id)) throw new Error("contract id already exists");
  ...
  transition(id: string, nextStatus: ContractStatus): ContractSnapshot {
    ...
    if (!allowedTransition(current.status, nextStatus)) throw new Error("invalid contract status transition");
...
function allowedTransition(current: ContractStatus, next: ContractStatus): boolean {
...
export function createContractForOrganization(
  ...
  if (role !== "owner" && role !== "admin") throw new Error("organization admin role required");
